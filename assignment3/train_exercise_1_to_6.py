...
def plot_loss(train_losses, valid_losses, save_path):
    ...
    plt.savefig(f"{save_path}.eps", dpi=300, bbox_inches='tight')
    plt.show()
...
def train_model(model, train_loader, valid_loader, optimizer, criterion, n_epochs, device, write_to_file=True, path_to_save_model=None):
    ...
    for epoch in range(n_epochs):
        ...
        print(f"Average train loss for epoch {epoch+1} is {train_loss}, validation loss is {valid_loss}")

        # write the model parameters to a file every 5 epochs
        if write_to_file and (epoch+1) % 10 == 0:
            torch.save(model.state_dict(), f"{path_to_save_model}_{epoch+1}_epochs.pth")

    if write_to_file:
        torch.save(model.state_dict(), f"{path_to_save_model}_best_{epoch+1}_epochs.pth")
