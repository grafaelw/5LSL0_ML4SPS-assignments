...
def scatter_plot(latent_tensor, label_tensor, n_points=10000):
    ...
    ax.set_xlabel('h0')
    ax.set_ylabel('h1')
    plt.savefig(f"figures/scatter-plot_{n_points}.png",dpi=300,bbox_inches='tight')
    plt.savefig(f"figures/scatter-plot_{n_points}.eps",dpi=300,bbox_inches='tight')
    plt.show()
...
def plot_images_exercise_1(x_data, model_output):
    ...
    plt.tight_layout()
    plt.savefig("figures/exercise_1.png",dpi=300,bbox_inches='tight')
    plt.savefig("figures/exercise_1.eps",bbox_inches='tight')
    plt.show() 
...
def plot_confusion_matrix(true_labels, output_labels, save_path=None):
    ...
    heatmap = sn.heatmap(df_cm, annot=True, cmap=plt.cm.Blues)
    ...
    # axis titles
    heatmap.set_ylabel('Ground-truth', fontsize = 18)
    heatmap.set_xlabel('Model-predicted', fontsize = 18)
...
def nearest_neighbour_exercise_3(train_latent, test_latent, train_labels, test_labels):
    ...
    # plot confusion matrix
    plot_confusion_matrix(test_labels, train_labels_neighbours, save_path='Figure/confusion_matrix_ex3')
...
def plot_mnist_grid_excercise_5(images, n_img_x, n_img_y, save_path=None):
    ...
    plt.savefig(f'{save_path}/exercise_5.eps', dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_images_exercise_6(noisy_images, model_out_images, clean_images, save_path=None):
    ...
    plt.savefig(f'{save_path}/exercise_6.eps', dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    AE = train.load_model(AE, "Models/Trained_clean/Model_AE_best_100_epochs.pth")
    ...
    #                                                     criterion_ex1, n_epochs, dev, 
    #                                                     write_to_file=True,
    #                                                     path_to_save_model= os.getcwd() + '/Models/Model_AE')

    # get latent vectors for excercise 3, use the trained model on the train set
    # losses_train, output_train, latent_train, label_train = train.test_model(AE, criterion_ex1, train_loader, dev)
    ...
    losses_test, output_test, latent_test, label_test = train.test_model(AE, criterion_ex1, test_loader, dev)
    ...
    # train.plot_loss(train_losses, valid_losses, save_path = "figures/loss_ex1")


    ### excercise 2: latent space ###
    scatter_plot(latent_tensor_test.cpu(), label_tensor_test.cpu(), n_points=1000)
    ...
    # mnist_classifier.to(dev)
    # optimizer_ex4 = optim.Adam(mnist_classifier.parameters(), learning_rate, weight_decay=1e-7)

    # mnist_classifier, train_losses, valid_losses = train.train_model(mnist_classifier, train_loader=train_loader, 
    #                                                     valid_loader=valid_loader, optimizer=optimizer_ex4, 
    #                                                     criterion=criterion_ex4, n_epochs=n_epochs, 
    #                                                     device=dev, 
    #                                                     write_to_file=True, 
    #                                                     path_to_save_model= 'Models/Classifier/Classifier')


    # plot the loss
    # train.plot_loss(train_losses, valid_losses, save_path='figures/Exercise-4_Classifier/excercise4_loss')

    # # load pretrained model
    # mnist_classifier = train.load_model(mnist_classifier, 'Models/Classifier/Classifier_best_20_epochs.pth')
    ...
    # # plot confusion matrix
    # plot_confusion_matrix(gt_tensor_test, pred_tensor_test, save_path='figures/Exercise-4_Classifier/confusion_matrix_class_ex4')
    ...
    # __, output_test, __, label_test = train.test_model(AE, criterion_ex1, test_loader, dev, use_noisy_images=True)
    ...
    #                        x_clean_example[:10], save_path='figures/Exercise_6')