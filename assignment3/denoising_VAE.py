...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.n.loc = self.n.loc.to('cpu')
        self.n.scale = self.n.scale.to('cpu')
...
def test():
    ...
    # test VAE
    x = torch.randn((64, 1, 32, 32)).to(torch.device("mps"))   
    model = VAE()

    output_decoder, x_sample, x_mean, x_log_varpreds = model(x).to(torch.device('mps'))
